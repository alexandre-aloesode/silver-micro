...
class Restaurant_Schedule_Model extends Abstract_Model
{
    public function __construct()
    {
        parent::connect();
        $this->api_helper = new API_Helper('silver-micro', 'restaurant_schedule');
        $this->status_helper = new Status_Helper();
        $this->tableName = 'restaurant_schedule';
    }

    public function postRestaurantSchedule($params)
    {
        $constraints = [
            ['restaurant_id', 'mandatory', 'number'], ['day_id', 'mandatory', 'number'], ['hour_id', 'mandatory', 'number']
        ];
    ...
    public function getRestaurantSchedule($params) {
        ...
        $constraints = [
            ['restaurant_schedule_id', 'optional', 'number'], ['restaurant_id', 'optional', 'number'],
            ['day_id', 'optional', 'number'], ['hour_id', 'optional', 'number'], 
        ];
        if ($this->api_helper->checkParameters($params, $constraints) == false)
        { return ($this->status_helper->PreconditionFailed()); }

        $fields = $this->getRestaurantScheduleFields();
    ...
    private function getRestaurantScheduleFields()
    {
        return ([
			'restaurant_schedule_id' => [
                'type' => 'in',
                'field' =>'id',
                'alias' => 'restaurant_schedule_id',
                'filter' => 'where'
            ],
            'restaurant_id' => [
                'type' => 'in',
